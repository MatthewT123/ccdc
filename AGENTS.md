...
The supported notebook-free sequence is `scripts/retrieve_structures.py` -> `scripts/predict_charges.py` -> `scripts/finetune.py`; for a fixed trusted CSD pool, use `scripts/create_csd_dataset.py` -> `scripts/batch_resp.py` -> `scripts/build_trusted_dataset.py` -> `scripts/assemble_labelled_splits.py`. See README for complete commands. Charge prediction here means reference RESP calculation using the existing `esp_generation.py`, not neural inference. These scripts leave notebooks unchanged and require fresh output directories.
...
`notebooks/reconstruction_and_charges.ipynb` is a separate executed visual-inspection notebook for ten deterministic molecules from the trusted 80/20 held-out test split. It loads the selected continuation checkpoint, uses supplied-geometry charge prediction only, and shows original structures colored by reference versus predicted charges plus full-range charge plots. It predicts no structures, runs no Psi4 or training, and exports PNG/JSON to fresh ignored `runs/reconstruction-inspection-*` directories. Use the ML GPU Python kernel; the original research notebooks remain unchanged.
...
| `scripts/create_csd_dataset.py` | Seeded, fixed CSD train/eval export with strict small-molecule quality filters, cross-split deduplication, and optional exclusion of prior SDF pools. |
| `scripts/assemble_labelled_splits.py` | Combines provenance-verified trusted pools into an exact train/test count while preserving SDF bytes and rejecting identifier, refcode-family, and connectivity overlap. |
...
- `scripts/create_csd_dataset.py --exclude-sdf` excludes identifiers, six-character refcode families, and stereo-independent connectivity identities from prior SDF pools. `scripts/assemble_labelled_splits.py` requires both input charge manifests to be provenance-verified, keeps the base trusted pool in train, and selects additional molecules deterministically from the supplied seed.
- Fine-tuning uses the existing joint reconstruction/charge/KL objective with constant CLI learning rates. `--trainable head` freezes pretrained parameters. Validation is deterministic charge MAE/RMSE on held-out molecules at decoder time 1, not a generative evaluation. Saved checkpoints can initialize a new run; optimizer/RNG state is not resumed by the CLI.
- Fine-tuning supports separate optimizer rates: `--lr`/`backbone_lr` covers pretrained encoder, latent, and structural-decoder parameters, while `--charge-lr` covers only `decoder.charge_head`. The charge rate defaults to the backbone rate for compatibility. Both groups are recorded in run/config metadata and local `steps.jsonl`; W&B stores them in run config while its compact history remains loss-only.
...
- Fine-tuning W&B history is intentionally compact: only weighted reconstruction loss and charge-prediction loss are logged per step/epoch/evaluation split; full diagnostics remain in local JSONL/JSON files and final performance is summary-only. `tests/test_wandb_logging.py` protects this selector.
...
- Trusted RESP expansion (2026-09-15): `runs/csd-small-v1-trusted-resp` completed all 1,000 original training SDFs in about 896 seconds with four CPU workers, `--precise-fit`, `--vdw-radius Br=1.85`, and a ±2 e raw/H-absorbed charge gate. `runs/csd-small-v1-trusted-v3` contains 588 byte-preserved, source-hash-verified SDFs and all-atom labels (9,085 atoms); 412 were removed for unstable RESP magnitudes and are listed with values in `status.csv`/`manifest.json`. `load_labels` verifies the filtered set. The original 100-molecule evaluation set was untouched. Use the filtered v3 directory for training; do not use the earlier pilot labels.
- Trusted dataset expansion (2026-09-15): `scripts/create_csd_dataset.py` screened 1,121,637 CSD entries and selected 1,200 fresh candidates while excluding all original train/eval identifiers, refcode families, and connectivity identities. `runs/csd-expansion-v1-resp` completed 1,200 RESP attempts in 1,103 seconds with four CPU workers; 710 passed the same precise-fit, Br-radius, and ±2 e raw/H-absorbed gate, while 490 were rejected and retained in `status.csv`. `scripts/assemble_labelled_splits.py` combined the 588 trusted base molecules with 412 additional trusted molecules into `runs/csd-small-v2-trusted-900x100`: exactly 900 train and 100 test, with source-hash verification and disjoint connectivity/refcode-family checks. The original 100 CSD eval structures remain excluded from this new test set to avoid leakage.
- Held-out training run (2026-09-15): `scripts/split_labelled_dataset.py` made the deterministic seed-42 80/20 split `runs/csd-small-v1-trusted-v3-split80-20` with 470 train and 118 test molecules. Ten GPU epochs with `--val-fraction 0`, batch size 16, and `--reconstruction-molecules 0` completed in 42.5 seconds (about 1.22 GiB peak PyTorch allocation). The W&B run is `unoxford/ccdc-molflae/rehgavyj`; epoch-10 evaluation losses were train reconstruction 4.3379, train charge 0.03625, test reconstruction 4.4649, and test charge 0.03569. The detailed per-epoch values are in `runs/trusted-split80-20-10epoch/metrics.jsonl`.
- Extended held-out run (2026-09-15): 20 additional GPU epochs initialized from `runs/trusted-split80-20-10epoch/last.ckpt` completed in 85.4 seconds as W&B `unoxford/ccdc-molflae/0zhd5ul3`. The minimum held-out combined objective was at additional epoch 15 (test reconstruction 4.2285, charge loss 0.03331, total objective 4.1254); at epoch 20 these were 4.3793, 0.03302, and 4.2670. `runs/trusted-split80-20-30epoch/best-test.ckpt` preserves epoch 15, `last.ckpt` preserves epoch 20, and `selection.json` records why the 20 per-epoch snapshots were pruned. The continuation used `--checkpoint-every-epoch`, which saves `epoch-XXXX.ckpt` snapshots before pruning.
- Differential-rate smoke (2026-09-15): one GPU epoch from `best-test.ckpt` with `--lr 1e-5 --charge-lr 1e-4` completed in `/tmp/ccdc-differential-lr-smoke`; `run.json`, `config.json`, and the first `steps.jsonl` record both rates and the charge head updated. `tests/test_devices.py` checks the two disjoint optimizer groups.
- Differential-rate continuation (2026-09-15): starting from `runs/trusted-split80-20-30epoch/best-test.ckpt`, 20 GPU epochs with backbone `1e-5` and charge-head `1e-4` completed as W&B `unoxford/ccdc-molflae/r7qrlzih`. Held-out reconstruction improved from 4.2280 to 4.1335 and charge loss from 0.03331 to 0.03301; the combined objective reached 4.0245 at epoch 20. `runs/trusted-split80-20-differential-lr-20epoch/best-test.ckpt` and `last.ckpt` both preserve the selected final state; `selection.json` records pruning of the 20 epoch snapshots. The geometry-only inspection notebook now loads this checkpoint.
