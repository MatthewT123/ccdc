...
- For notebook GPU training, set `config['decoder_config_charge']['device'] = 'cuda'` before `TrainLoopCharges(config).to('cuda')`, move batches to CUDA, then initialize the optimizer after moving the model. Some decoder device state is not a registered buffer, so `.to('cuda')` alone is insufficient for all paths. Full GPU sampling/training remains unverified beyond the smoke update.
