...
class SinusoidalPosEmb(nn.Module):
    ...
    Args:
        dim (int): Dimensionality of the output embedding. Must be even.

    Input:
        x (Tensor): A 1D tensor of shape (batch_size,) containing scalar inputs.

    Output:
        Tensor: A tensor of shape (batch_size, dim) containing sinusoidal
        embeddings of the input scalars.
    """
    ...
    def forward(self, x):
        ...
        # Split the input into sine and cosine parts
        half_dim = self.dim // 2

        # Construct log-spaced frequencies
...
class RBF(nn.Module):
    """
    Radial basis function (RBF) expansion for continuous scalar inputs

    Args:
        start (float): Lower bound of the RBF center range.
        end (float): Upper bound of the RBF center range.
        n_center (int): Number of radial basis function centers.

    Input:
        x (Tensor): A tensor of shape (..., 1) or (..., D) containing
            real-valued inputs. The last dimension is expanded into
            `n_center` RBF features.

    Output:
        Tensor: A tensor of shape (..., n_center) containing the RBF-expanded
...
class TimeEmbedLayer(nn.Module):
    """
    COnfigurable time-embedding layer
    """
    def __init__(self, time_emb_mode, time_emb_dim):
        ...
        if self.time_emb_mode == "simple":  # identity embedding
            assert self.time_emb_dim == 1
            self.time_emb = lambda x: x

        elif self.time_emb_mode == "sin":  # sinusoidal time embedding + MLP
        ...
        elif self.time_emb_mode == "rbf":  # radial basis function embedding
            self.time_emb = RBF(0, 1, self.time_emb_dim)

        elif self.time_emb_mode == "rbfnn":  # radial basis function embedding + MLP
            self.time_emb = nn.Sequential(
...
class BFN4SBDDScoreModel(BFNBase):
    ...
    def interdependency_modeling(
        ...
        if self.node_indicator:  # Add node indicator channel
            h_protein = torch.cat(
    ...
    def loss_one_step(
        ...
        # Predict discrete one-hot labels
...
class BFN_charge(BFNBase):
    def __init__(
        ...
        super(BFN_charge, self).__init__()
        ...
        # Instantiate the transformer network
        if net_config.name == 'unio2net':
            # make a shallow copy of dict and remove keys UniTransformer doesn't expect
            unio_args = net_config.todict().copy()

            # Remove keys that are not constructor args for UniTransformer
            for _k in [
                "include_charge",
                "charge_discretised_loss",
                "self.charge_loss_weight",
                "sigma1_charges",
                "charge_loss_weight",
                "field_loss_weight",
                "total_charge_weight",
            ...
            # Instantiate the transformer with the cleaned args
            self.unio2net = UniTransformerO2TwoUpdateGeneral(**unio_args)
        else:
            raise NotImplementedError
        ...
        if self.include_charge:
            # MLP head to predict per-atom continuous charges from final_h
    ...
    def loss_one_step(
        ...
        if self.include_charge:
            # compute Bayesian update parameters for charges (continuous)
            mu_charge, gamma_charge = self.continuous_var_bayesian_update(
                t, sigma1=self.sigma1_charges, x=ligand_charges
            )  # shapes: [N_ligand,1], [N_ligand,1]
        else:
            mu_charge = None
            gamma_charge = None
        ...
        if self.include_charge:
            ...
            # Add soft total-charge penalty per molecule:
            # sum predicted charges per molecule
            q_pred_sum = segment_sum(k_hat.squeeze(-1), batch_ligand)  # [B] per-molecule sum
            # expected/true total charge per molecule
    ...
    def sample(
        ...
        # Charge prior
        if self.include_charge:
            mu_charge_t = torch.zeros((n_nodes, 1)).to(self.device)  # prior mean 0
        else:
            mu_charge_t = None

        theta_h_t = torch.ones((n_nodes, K)).to(self.device) / K
        ro_coord = 1.0
        ro_charge = 1.0

        ro_coord = torch.tensor(1.0, device=self.device)
        ro_charge = torch.tensor(1.0, device=self.device)
        ...
        theta_h_t = theta_h_t[batch_ligand]
        if self.include_charge:
            mu_charge_t = mu_charge_t[batch_ligand]  
        ...
        for i in trange(1, sample_steps + 1, desc=f'{desc}'):
            ...
            if self.include_charge and k_hat is not None:
                if k_hat.dim() == 1:
            ...
            if self.sampling_strategy == "vanilla":
                ...
                # prepare next-step time t_next (as in original code)
                t_next = torch.ones((n_nodes, 1), device=self.device) * i / sample_steps
                t_next = t_next[batch_ligand]  # index to ligand nodes

                # update discrete theta_h_t depending on sampling strategy variant
                if self.sampling_strategy == "end_back":
                    theta_h_t = self.discrete_var_bayesian_update(
                        t_next, beta1=self.beta1, x=sample_pred, K=K
                    )
                elif self.sampling_strategy == "end_back_pmf":
                    theta_h_t = self.discrete_var_bayesian_update(
                        t_next, beta1=self.beta1, x=p0_h_pred, K=K
                    )
                elif self.sampling_strategy == "end_back_mode":
                    p0_mode = torch.argmax(p0_h_pred, dim=-1)
                    mode_pred = F.one_hot(p0_mode, num_classes=K).float()
                    theta_h_t = self.discrete_var_bayesian_update(
                        t_next, beta1=self.beta1, x=mode_pred, K=K
                    )
                else:
                    raise NotImplementedError(f"sampling strategy {self.sampling_strategy} not implemented")
                ...
                if self.include_charge and not self.charge_discretised_loss:
                    ...
                    # Use the same continuous update helper but for charges
                    mu_charge_t = self.continuous_var_bayesian_update(
                        t_next, sigma1=self.sigma1_charges, x=k_hat
                    )[0]
...
def segment_sum(values, segment_ids):
    # values: [N_nodes] or [N_nodes, ...], segment_ids: same shape giving 0..B-1
    return torch_scatter.scatter_add(values, segment_ids, dim=0)


def segment_mean(values, segment_ids):
    s = segment_sum(values, segment_ids)
    counts = torch_scatter.scatter_add(torch.ones_like(values), segment_ids, dim=0)
    return s / (counts.clamp(min=1.0))
